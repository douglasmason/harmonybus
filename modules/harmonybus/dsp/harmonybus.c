/* Harmony Bus v0.2.24 — Schwung MIDI FX. */
#define HB_VERSION "0.2.24"
...
static int hb_map_follower_note_now(Inst *instance,int source_note){
    ...
    if(!harmony.valid||g_bus.follower_content_map==2)return source_note; /* Free */
    if(g_bus.follower_content_map==1)harmony=hb_follower_scale_target(instance,harmony);
    else harmony=hb_mapping_target(harmony,0);
    if(g_bus.follower_travel_map==1)
        return hb_map_note(source_note,reference_root(instance),harmony,HB_MAP_NEAREST);
    /* Relative: preserve the follower's chromatic interval from its reference
       root, then express that interval from the conductor harmony root. If the
       exact degree is unavailable in Chord/Scale content, hb_map_note chooses
       the nearest legal degree around the relative target. */
    int used_root=0;
...
static hb_harmony_t hb_follower_scale_target(Inst *instance,hb_harmony_t harmony){
    ...
    harmony.bass_pc=harmony.root_pc;
    harmony.pitch_mask=hb_explicit_scale_mask(harmony.root_pc,g_bus.follower_scale);
    return harmony;
}
static hb_harmony_t hb_mapping_target(hb_harmony_t harmony,int map_target){
    if(!harmony.valid)return harmony;
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9)&&values[0]==0&&!instance->global_timing_restored){
        ...
        if((is_hb14||is_hb13)&&values[23]>=0&&values[23]<10)g_bus.follower_scale=values[23];
        if(is_hb14&&values[21]>=0&&values[21]<3)g_bus.follower_content_map=values[21];
        if(is_hb14&&values[22]>=0&&values[22]<2)g_bus.follower_travel_map=values[22];
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB224");
