/* Harmony Bus v0.2.09 — Schwung MIDI FX. */
#define HB_VERSION "0.2.09"
...
static int tick(void *value,int frames,int sample_rate,uint8_t output[][3],int lengths[],int max_output){
    ...
    int complete_live_free=(g_bus.chord_timing==0&&g_bus.context==0&&count>=3);
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB209");
