...
typedef struct { volatile unsigned seq; hb_harmony_t harmony; int global_transpose; int global_root_policy; int global_explicit_root; int global_input_root; int chord_timescale; int stability; int chord_timing; int context; int accidentals; int auto_spell_sharps; int auto_spell_locked; int clip_track; int clip_valid; int clip_note_count; int clip_stage; int clip_context; int last_clock_status; double clip_loop_start; double clip_loop_end; unsigned long clip_clock_ticks; unsigned clip_refresh_counter; double last_clip_playhead; int have_last_clip_playhead; hb_clip_note_t clip_notes[HB_MAX_CLIP_NOTES]; } SharedBus;
...
static int hb_load_clip_cache(void){
    ...
    while(scan<slots_end){
        ...
        if(!clip||clip>=slots_end)break;
        const char *obj=strstr(clip,"{");
