/* Harmony Bus v0.1.84 — Schwung MIDI FX. */
#define HB_VERSION "0.1.84"
...
typedef struct { int used,role,mode,window_ms,dirty,frames_since_change; uint8_t active[128]; uint8_t held_now[128]; uint8_t held_count[128]; int pending_off_frames[128]; int mapped[128]; uint8_t follower_held[128]; uint8_t follower_velocity[128]; unsigned follower_bus_seq; hb_harmony_t follower_source_harmony; uint8_t source_seen[12]; int resolved_root,resolved_confidence; unsigned rx_count; unsigned note_on_count; unsigned note_off_count; int last_note; int last_status; int last_velocity; int active_count; int last_inferred_count; unsigned raw_event_count; unsigned raw_note_count; unsigned raw_note_on_count; unsigned raw_note_off_count; int raw_last_note; int raw_last_status; int raw_last_velocity; int raw_last_channel; int raw_last_cable; uint8_t raw_prev[HB_MIDI_OUT_BYTES]; int map_target; hb_harmony_t candidate_harmony; int candidate_frames; int committed_frames; int render_channel; int source_channel; int resolved_source_channel; unsigned live_press_count; int live_vouch_pending; int live_vouch_age; int recent_live_note[16]; int recent_live_age[16]; uint8_t recent_live_valid[16]; unsigned render_count; unsigned render_fail_count; int render_last_note; int retrigger_held; uint8_t follower_role_interval[128]; uint8_t published_conductor[128]; uint8_t published_follower[128]; int settle_frames_remaining; int clip_event_idle_frames; int last_transport_playing; uint8_t trace_note[8]; uint8_t trace_on[8]; uint8_t trace_channel[8]; unsigned trace_count; int follower_source_policy; int follower_source_root; int follower_source_quality; } Inst;
...
static hb_harmony_t hb_make_fixed_harmony(int root_pc,int quality){
    uint8_t notes[3];
    int root=mod12(root_pc);
    notes[0]=(uint8_t)(60+root);
    notes[1]=(uint8_t)(60+root+(quality?3:4));
    notes[2]=(uint8_t)(60+root+7);
...
static hb_harmony_t hb_resolve_follower_source_harmony(Inst *instance){
    ...
    if(!instance)return harmony;
    if(instance->follower_source_policy==0){
        return hb_make_fixed_harmony(g_bus.global_input_root,instance->follower_source_quality);
    }
    if(instance->follower_source_policy==2){
        return hb_make_fixed_harmony(instance->follower_source_root,instance->follower_source_quality);
    }
    uint8_t notes[128];int count=0;
    for(int note=0;note<128;note++)if(instance->follower_held[note])notes[count++]=(uint8_t)note;
    if(count>=2){
        harmony=hb_infer_harmony(notes,count);
        if(harmony.valid)return harmony;
    }
    if(instance->follower_source_harmony.valid)return instance->follower_source_harmony;
    return hb_make_fixed_harmony(g_bus.global_input_root,instance->follower_source_quality);
}
...
static int hb_reharmonize_held_follower(Inst *instance,uint8_t output[][3],int lengths[],int max_output){
    ...
    for(int source_note=0;source_note<128;source_note++){
        ...
        instance->follower_source_harmony=hb_resolve_follower_source_harmony(instance);int next_role=instance->follower_source_harmony.valid?mod12(source_note-instance->follower_source_harmony.root_pc):255;
...
instance->rx_count++;int status=input[0]&0xF0,is_on=(status==0x90&&length>=3&&input[2]>0),is_off=(status==0x80&&length>=3)||(status==0x90&&length>=3&&input[2]==0);
