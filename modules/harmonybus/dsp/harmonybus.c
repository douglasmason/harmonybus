/* Harmony Bus v0.2.54 — Schwung MIDI FX. */
#define HB_VERSION "0.2.54"
...
static hb_harmony_t hb_follower_content_target(Inst *instance,hb_harmony_t harmony,int content_map){
    ...
    */
    ...
    if(content_map<3||content_map>6)return target;
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if((is_hb15||is_hb14)&&values[0]==0&&!g_follower_globals_restored){
        ...
        if(values[21]>=0&&values[21]<7)hb_set_global_content_map(values[21]);
...
if(!strcmp(key,"content_map")){
    static const char *opts[]={"In Chord","In Scale","Free","In 135","In 1357","In 12357","In 12356"};
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB254");
