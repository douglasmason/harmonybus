...
static int hb_observed_notes(const Inst *instance,uint8_t *output,int max_notes){
    ...
    int count=0;
    /*
     * Only authoritative, current conductor events may become note evidence.
     * Do not merge Song.abl/set-state snapshots here: those files can lag the
     * live Set and previously allowed deleted clip notes to survive as ghosts.
     * Clip playback and live playing are intentionally unified at the MIDI
     * event stream seen by the Conductor instance.
     */
    for(int note=0;note<128&&count<max_notes;note++){
        if(instance->held_now[note])output[count++]=(uint8_t)note;
    }
    return count;
}
