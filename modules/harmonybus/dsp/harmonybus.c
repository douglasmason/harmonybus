/* Harmony Bus v0.2.34 — Schwung MIDI FX. */
#define HB_VERSION "0.2.34"
...
static int hb_scale_relative_interval(int source_interval,int scale_index){
    ...
    /* Infer has no fixed source scale. Preserve the chromatic degree exactly;
       e.g. F->G is +2 and therefore maps to +2 from every target harmony root. */
    if(scale_index<=0||scale_index>9)return mod12(source_interval);
    const int *scale=intervals[scale_index-1];
    int best=scale[0],best_distance=99;
    for(int index=0;index<7;index++){
        int distance=mod12(source_interval-scale[index]);
        if(distance>6)distance=12-distance;
        if(distance<best_distance){best_distance=distance;best=scale[index];}
    }
    return best;
}
static int hb_map_scale_degree_relative(Inst *instance,int source_note,hb_harmony_t harmony){
    ...
    int interval=hb_scale_relative_interval(mod12(source_note-source_root),hb_global_scale());
    int target_pc=mod12(harmony.root_pc+interval);
    int nominal=source_note+mod12(harmony.root_pc-source_root);
    if(mod12(harmony.root_pc-source_root)>6)nominal-=12;
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB234");
