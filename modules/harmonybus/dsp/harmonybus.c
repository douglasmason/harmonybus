/* Harmony Bus v0.2.25 — Schwung MIDI FX. */
#define HB_VERSION "0.2.25"
...
static int hb_global_content_map(void){
    hb_global_open();
    return g_global_shared?g_global_shared->follower_content_map:0;
}
static int hb_global_travel_map(void){
    hb_global_open();
    return g_global_shared?g_global_shared->follower_travel_map:0;
}
static int hb_global_scale(void){
    hb_global_open();
    return g_global_shared?g_global_shared->follower_scale:0;
}
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9)&&values[0]==0&&!instance->global_timing_restored){
        ...
        if((is_hb14||is_hb13)&&values[23]>=0&&values[23]<10)hb_set_global_scale(values[23]);
        if(is_hb14&&values[21]>=0&&values[21]<3)hb_set_global_content_map(values[21]);
        if(is_hb14&&values[22]>=0&&values[22]<2)hb_set_global_travel_map(values[22]);
...
if(!strcmp(key,"travel_map")){static const char *opts[]={"Relative","Closest"};return snprintf(buffer,(size_t)length,"%s",opts[hb_global_travel_map()]);}
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB225");
