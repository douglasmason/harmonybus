...
static int hb_observed_notes(const Inst *instance,uint8_t *output,int max_notes){
    ...
    if(g_bus.clip_valid&&g_bus.clip_note_count>0){
    ...
    /* Live evidence is instance-local. process() only writes held_now[] for a
       Conductor instance, so improvisation routed to followers cannot leak in. */
