/* Harmony Bus v0.1.96 — Schwung MIDI FX. */
#define HB_VERSION "0.1.96"
...
static int hb_infer_follower_root(Inst *instance,int *root_out){
    ...
    if(g_bus.follower_root_policy==0){
        hb_monitor_open();
        if(!g_monitor)return 0;
        int have_root=0;
        int root=-1;
    ...
    if(g_bus.follower_root_policy==1){
        uint8_t seen[128];memset(seen,0,sizeof(seen));
...
static int hb_resolve_follower_reference_root(Inst *instance,int *root_out){
    (void)instance;
    if(!root_out)return 0;
    if(g_bus.follower_root_policy==2){
        *root_out=mod12(g_bus.follower_explicit_root);
        return 1;
    }
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if(parsed==18){
        if(values[16]>=0&&values[16]<=2)g_bus.follower_root_policy=values[16];
        if(values[17]>=0&&values[17]<12)g_bus.follower_explicit_root=values[17];
    }
    if(parsed!=18){
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB196");
