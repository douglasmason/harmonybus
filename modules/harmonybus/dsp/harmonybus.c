/* Harmony Bus v0.1.64 — Schwung MIDI FX. */
#define HB_VERSION "0.1.64"
...
static int hb_observed_notes(const Inst *instance,uint8_t *output,int max_notes){
    ...
    int count=0;
    /* Realtime MIDI is the sole production inference source for now.
       Clip parsing/cache code remains available for diagnostics and future
       re-enablement, but cached clip notes must never contaminate harmony
       classification while that path is under repair. */
