/* Harmony Bus v0.2.56 — Schwung MIDI FX. */
#define HB_VERSION "0.2.56"
...
static hb_harmony_t hb_follower_content_target(Inst *instance,hb_harmony_t harmony,int content_map){
    ...
    */
    ...
    if(content_map<3||content_map>7)return target;
...
static int hb_map_follower_note_closest_meta(Inst *instance,int source_note,hb_harmony_t detected,
                                                  hb_harmony_t content_target){
    /* Preserve the follower's metrical/harmonic intent:
         - source ROLE represented by an actually played conductor chord tone
             -> choose the closest ACTUAL conductor chord tone;
         - otherwise
             -> choose the closest IN-SCALE NON-CHORD tone.

       "Role" is ordinal scale degree, not chromatic interval quality. Input
       role is derived from follower root + follower parent scale; conductor
       role is derived from the detected harmony's chord-scale. */
    int source_root=0;
    ...
    uint16_t played_conductor_mask=0;
    uint8_t conductor_notes[64];
    int conductor_count=hb_observed_notes(0,conductor_notes,64);
    for(int index=0;index<conductor_count;index++)
        played_conductor_mask|=(uint16_t)(1u<<mod12(conductor_notes[index]));

    uint16_t actual_chord_mask=(uint16_t)(played_conductor_mask&hb_harmony_chord_mask(detected));
    int role_has_actual_chord_tone=0;
    for(int degree=0;degree<7;degree++){
        if(degree!=source_degree)continue;
        int interval=hb_nth_scale_interval_from_root(chord_scale,detected.root_pc,degree);
        int pc=mod12(detected.root_pc+interval);
        if(actual_chord_mask&(1u<<pc))role_has_actual_chord_tone=1;
    }

    hb_harmony_t meta_target=content_target;
    if(role_has_actual_chord_tone){
        meta_target.pitch_mask=actual_chord_mask;
    }else{
        /* Off-beat/meta-scale case: deliberately exclude inferred and played
           chord tones. Respect the selected Follower Content mode by
           intersecting its target set with the detected chord-scale. */
        uint16_t scale_mask=chord_scale?chord_scale:hb_scale_mask(detected);
        meta_target.pitch_mask=(uint16_t)(content_target.pitch_mask&scale_mask&
                                          (uint16_t)(~hb_harmony_chord_mask(detected)));
        meta_target.pitch_mask&=0x0FFFu;
        if(!meta_target.pitch_mask){
            /* If a restrictive Content mode offers no non-chord scale tone,
               fall back to any non-chord tone in the detected chord-scale. */
            meta_target.pitch_mask=(uint16_t)(scale_mask&
                                              (uint16_t)(~hb_harmony_chord_mask(detected))&
                                              0x0FFFu);
        }
    }
    if(!meta_target.pitch_mask)
        return hb_map_note(source_note,reference_root(instance),content_target,HB_MAP_NEAREST);
    return hb_map_note(source_note,reference_root(instance),meta_target,HB_MAP_NEAREST);
}
static int hb_map_follower_note_now(Inst *instance,int source_note){
    hb_harmony_t harmony=bus_read();
    int content_map=hb_global_content_map();
    if(!harmony.valid||content_map==2)return source_note; /* Free */

    /* In Scale + Relative transports ordinal scale degree directly. */
    if(content_map==1&&hb_global_travel_map()==0)
        return hb_map_scale_degree_relative(instance,source_note,harmony);

    hb_harmony_t detected=harmony;
    if(content_map==1)harmony=hb_follower_scale_target(instance,harmony);
    else harmony=hb_follower_content_target(instance,harmony,content_map);

    if(hb_global_travel_map()==3)
        return hb_map_follower_note_closest_meta(instance,source_note,detected,harmony);
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if((is_hb15||is_hb14)&&values[0]==0&&!g_follower_globals_restored){
        ...
        if(values[21]>=0&&values[21]<8)hb_set_global_content_map(values[21]);
        if(values[22]>=0&&values[22]<4)hb_set_global_travel_map(values[22]);
...
if(!strcmp(key,"content_map")){
    static const char *opts[]={"In Chord","In Scale","Free","In 135","In 1357","In 12357","In 12356","In Non-Avoid"};
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB256");
