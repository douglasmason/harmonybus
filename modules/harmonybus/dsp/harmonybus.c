...
"]";
static void set_param(void *value,const char *key,const char *parameter){Inst *instance=(Inst*)value;if(!instance||!key||!parameter)return;if(!strcmp(key,"live_press")){if(parameter[0]=='1')hb_receive_live_vouch(instance);return;}if(!strcmp(key,"approach_below_pad")){int down=parameter[0]=='1';if(g_bus.approach_mode==0){if(down)instance->approach_pad_armed=0;}else instance->approach_below_held=(uint8_t)down;return;}if(!strcmp(key,"approach_above_pad")){int down=parameter[0]=='1';if(g_bus.approach_mode==0){if(down)instance->approach_pad_armed=2;}else instance->approach_above_held=(uint8_t)down;return;}if(!strcmp(key,"approach_reset")){if(parameter[0]=='1'||!strcmp(parameter,"Reset")){instance->approach_pad_armed=1;instance->approach_below_held=0;instance->approach_above_held=0;g_bus.approach_control=1;}return;}if(!strcmp(key,"mod_scale_above")){int down=parameter[0]=='1'||!strcmp(parameter,"On");if(g_bus.approach_mode==0){if(down)instance->approach_pad_armed=2;}else instance->approach_above_held=(uint8_t)down;return;}if(!strcmp(key,"mod_chrom_below")){int down=parameter[0]=='1'||!strcmp(parameter,"On");if(g_bus.approach_mode==0){if(down)instance->approach_pad_armed=0;}else instance->approach_below_held=(uint8_t)down;return;}if(!strcmp(key,"track_role")||!strcmp(key,"role")){
...
static int get_param(void *value,const char *key,char *buffer,int length){Inst *instance=(Inst*)value;if(!instance||!key||!buffer||length<2)return -1;hb_harmony_t harmony=bus_read();if(!strcmp(key,"version"))return snprintf(buffer,(size_t)length,"%s",HB_VERSION);if(!strcmp(key,"approach_status")){int active=hb_active_approach(instance);return snprintf(buffer,(size_t)length,"%s",APPROACH_OPTS[(active>=0&&active<3)?active:1]);}if(!strcmp(key,"touch_hint"))return snprintf(buffer,(size_t)length,"K6 OFF K7 UP K8 DN");if(!strcmp(key,"approach_reset"))return snprintf(buffer,(size_t)length,"Off");if(!strcmp(key,"mod_scale_above")){int on=g_bus.approach_mode==0?(instance->approach_pad_armed==2):(instance->approach_above_held!=0);return snprintf(buffer,(size_t)length,"%s",on?"On":"Off");}if(!strcmp(key,"mod_chrom_below")){int on=g_bus.approach_mode==0?(instance->approach_pad_armed==0):(instance->approach_below_held!=0);return snprintf(buffer,(size_t)length,"%s",on?"On":"Off");}if(!strcmp(key,"approach"))return snprintf(buffer,(size_t)length,"%s",APPROACH_OPTS[(g_bus.approach_control>=0&&g_bus.approach_control<3)?g_bus.approach_control:1]);if(!strcmp(key,"approach_mode"))return snprintf(buffer,(size_t)length,"%s",APPROACH_MODE_OPTS[g_bus.approach_mode?1:0]);if(!strcmp(key,"approach_below_pad"))return snprintf(buffer,(size_t)length,"%d",instance->approach_below_held?1:0);if(!strcmp(key,"approach_above_pad"))return snprintf(buffer,(size_t)length,"%d",instance->approach_above_held?1:0);if(!strcmp(key,"retrigger_held"))return snprintf(buffer,(size_t)length,"%s",RETRIGGER_OPTS[instance->retrigger_held?1:0]);if(!strcmp(key,"follow_lookahead_ms"))return snprintf(buffer,(size_t)length,"%d",instance->follow_lookahead_ms);if(!strcmp(key,"conductor_this_1")){char b[8];return snprintf(buffer,(size_t)length,"%s",hb_note_name_with_octave(hb_nth_local_conductor_note(instance,0),harmony,b,sizeof(b)));}
