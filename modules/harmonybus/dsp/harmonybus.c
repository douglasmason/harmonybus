/* Harmony Bus v0.2.02 — Schwung MIDI FX. */
#define HB_VERSION "0.2.02"
...
static int hb_source_channel_matches(Inst *instance,int midi_channel){
    ...
    /* Auto means trust the parent Chain's per-track routing. The host
       slot_recv_channel callback expects the parent Chain instance, not this
       MIDI-FX subplugin pointer, so using it here cross-wires tracks. */
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB202");
