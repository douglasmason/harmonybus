/* Harmony Bus v0.2.39 — Schwung MIDI FX. */
#define HB_VERSION "0.2.39"
...
static int hb_queue_follower_event(Inst *instance,int note,int velocity,int is_on,int channel){
    ...
    if(is_on){
        ...
        /* Harmonic capture: a follower note shortly before an anticipated
           chord boundary waits for that boundary so it maps against the new
           harmony. */
        double chord_grid=hb_chord_grid_beats();
        if(chord_grid>0.0){
            double offset=hb_anticipation_beats();
            long bucket=(long)(beat/chord_grid);
            double boundary=(double)(bucket+1)*chord_grid-offset;
            if(boundary<=beat+1e-9)boundary+=chord_grid;
            double distance=boundary-beat;
            if(distance>=-1e-6&&distance<=capture+1e-6)target=boundary;
        }

        /* Optional rhythmic quantization is deliberately capture-only, not a
           conventional always-on quantizer. Notes outside the final Buffer
           window before a Quant Grid tick are untouched. If both chord capture
           and quantization apply, wait for the later boundary. */
        double quant_grid=hb_quant_grid_beats();
        if(quant_grid>0.0){
            long bucket=(long)(beat/quant_grid);
            double boundary=(double)(bucket+1)*quant_grid;
            if(boundary<=beat+1e-9)boundary+=quant_grid;
            double distance=boundary-beat;
            if(distance>=-1e-6&&distance<=capture+1e-6&&boundary>target)
                target=boundary;
        }
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if((is_hb15||is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9||parsed==13||parsed>=15)&&values[12]>=-1&&values[12]<16)instance->source_channel=values[12];
    ...
    if((is_hb15||is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9)&&values[0]==0&&!instance->global_timing_restored){
        ...
        if((is_hb15||is_hb14||is_hb13||is_hb12||is_hb11||is_hb10)&&values[19]>=0&&values[19]<=500)g_bus.boundary_buffer_ms=values[19];
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB239");
