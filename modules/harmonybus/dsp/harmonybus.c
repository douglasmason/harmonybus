/* Harmony Bus v0.2.57 — Schwung MIDI FX. */
#define HB_VERSION "0.2.57"
...
static int hb_map_follower_note_closest_meta(Inst *instance,int source_note,hb_harmony_t detected,
    ...
    /* "Closest Meta" preserves only the meta distinction:
         IN-CHORD               -> nearest inferred chord tone
         OUT-OF-CHORD-IN-SCALE  -> nearest non-chord tone in the inferred chord-scale

       It is deliberately independent of the literal conductor voicing. The
       conductor is used to INFER harmony, but whether a particular chord tone
       was physically played does not matter.

       The source note's ordinal role still comes from follower root + follower
       scale. We then ask whether that SAME ordinal degree is a chord tone of
       the inferred harmony:
         triad:   1/3/5 are in-chord; 2/4/6/7 are out-of-chord-in-scale
         seventh: 1/3/5/7 are in-chord; 2/4/6 are out-of-chord-in-scale
       Extended/altered harmonies generalize from the inferred chord mask. */
    (void)content_target;
    int source_root=0;
    ...
    uint16_t chord_mask=hb_harmony_chord_mask(detected);
    ...
    int source_role_is_chordal=(chord_mask&(1u<<degree_pc))!=0;

    hb_harmony_t meta_target=detected;
    if(source_role_is_chordal){
        meta_target.pitch_mask=chord_mask;
    }else{
        meta_target.pitch_mask=(uint16_t)(chord_scale&
                                          (uint16_t)(~chord_mask)&
                                          0x0FFFu);
        if(!meta_target.pitch_mask){
            /* Degenerate fallback: if no non-chord scale tones exist, use the
               inferred chord rather than emitting an unconstrained pitch. */
            meta_target.pitch_mask=chord_mask;
        }
    }
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB257");
