...
static int tick(void *value,int frames,int sample_rate,uint8_t output[][3],int lengths[],int max_output){
    ...
    /* Always keep the candidate synchronized with what the current active
       notes imply. Commitment timing/hysteresis is applied only after this. */
    ...
    /* A complete new voicing must never be vetoed merely because the previous
       harmony happened to contain some of its notes. That was the source of
       the observed "stuck at G#5" behavior. */
    if(candidate.valid){
        int min_dwell=hb_timescale_frames(sample_rate);
