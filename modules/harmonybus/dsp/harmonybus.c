...
static int hb_find_conductor_track(const char *uuid){
    ...
    for(int track=0;track<4;track++){
        ...
        int is_conductor=(strstr(json,"hb5,0,")||strstr(json,"hb4,0,")||strstr(json,"hb3,0,")||strstr(json,"hb2,0,")||strstr(json,"hb1,0,"));
        free(json);
...
static int hb_observed_notes(const Inst *instance,uint8_t *output,int max_notes){
    if(!instance||!output||max_notes<=0)return 0;
    uint8_t seen[128];memset(seen,0,sizeof(seen));int count=0;
    int clip_has_content=g_bus.clip_valid&&g_bus.clip_note_count>0;
    if((instance->source_mode==0||instance->source_mode==2)&&clip_has_content){
        uint8_t clip_notes[32];int clip_count=hb_clip_active_notes(clip_notes,32);
        for(int i=0;i<clip_count&&count<max_notes;i++){
            int note=clip_notes[i];if(note>=0&&note<128&&!seen[note]){seen[note]=1;output[count++]=(uint8_t)note;}
        }
    }
    /* "Clip" is clip-preferred, not clip-or-silence: when there is no stored
       material to inspect, fall back to the live held-note observation. */
    int use_live=(instance->source_mode==1||instance->source_mode==2||
                  (instance->source_mode==0&&!clip_has_content));
    if(use_live){
        for(int note=0;note<128&&count<max_notes;note++){
            if(instance->held_now[note]&&!seen[note]){seen[note]=1;output[count++]=(uint8_t)note;}
        }
    }
    for(int i=0;i<count;i++)for(int j=i+1;j<count;j++)if(output[j]<output[i]){uint8_t temp=output[i];output[i]=output[j];output[j]=temp;}
    return count;
}
...
static int tick(void *value,int frames,int sample_rate,uint8_t output[][3],int lengths[],int max_output){
    ...
    if(g_bus.clip_valid||instance->source_mode==1){
