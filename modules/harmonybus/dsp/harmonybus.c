/* Harmony Bus v0.2.35 — Schwung MIDI FX. */
#define HB_VERSION "0.2.35"
...
static int hb_find_conductor_track(const char *uuid){
    ...
    for(int track=0;track<4;track++){
        ...
        int is_conductor=(strstr(json,"hb14,0,")||strstr(json,"hb13,0,")||strstr(json,"hb12,0,")||strstr(json,"hb11,0,")||strstr(json,"hb10,0,")||strstr(json,"hb9,0,")||strstr(json,"hb8,0,")||strstr(json,"hb7,0,")||strstr(json,"hb6,0,")||strstr(json,"hb5,0,")||strstr(json,"hb4,0,")||strstr(json,"hb3,0,")||strstr(json,"hb2,0,")||strstr(json,"hb1,0,"));
        free(json);
...
static int hb_queue_follower_event(Inst *instance,int note,int velocity,int is_on,int channel){
    ...
    /* If the follower lands shortly BEFORE an expected harmonic boundary,
       capture it for that boundary. This is beat-aware anticipation: e.g.
       Grid=1/4, Anticipation=1/32 means a note in the final 1/32 before the
       quarter-note boundary waits for the chord arriving on that boundary. */
    if(is_on){
        double grid=hb_chord_grid_beats();
        if(grid>0.0){
            double beat=hb_current_beat();
            double offset=hb_anticipation_beats();
            double capture=hb_ms_to_beats(g_bus.boundary_buffer_ms);
            /* Expected harmonic changes repeat every grid interval, shifted
               earlier by Anticipation. Follower Buffer is independent and
               only defines the small pre-change capture window. */
            long bucket=(long)(beat/grid);
            double boundary=(double)(bucket+1)*grid-offset;
            if(boundary<=beat+1e-9)boundary+=grid;
            double distance=boundary-beat;
            if(distance>=-1e-6&&distance<=capture+1e-6)
                instance->follower_queue_target_beat[slot]=boundary;
        }
    }
...
static int hb_source_interval_to_diatonic_degree(int source_interval){
    /* In Scale is diatonic-role mapping, not chromatic transposition.
       Collapse altered spellings onto the same scale degree:
       b2/2 -> 2, m3/M3 -> 3, 4/#4 -> 4, 5 -> 5,
       b6/6 -> 6, m7/M7 -> 7. */
    switch(mod12(source_interval)){
        case 0:return 0;       /* root / octave */
        case 1:case 2:return 1;/* 9th / 2nd */
        case 3:case 4:return 2;/* 3rd */
        case 5:case 6:return 3;/* 11th / 4th */
        case 7:return 4;       /* 5th */
        case 8:case 9:return 5;/* 13th / 6th */
        case 10:case 11:return 6;/* 7th */
    }
    return 0;
}
...
static int hb_inferred_parent_scale_index(int source_root,hb_harmony_t harmony){
    /* Infer the tonic scale quality from how well the detected chord fits the
       candidate parent scale. Major vs natural minor is intentionally the
       conservative automatic choice; explicit modes remain available for
       Dorian/etc. Ties default to Major, which is stable for sparse C/C5-style
       conductor information until stronger evidence arrives. */
    uint16_t chord=hb_harmony_chord_mask(harmony);
    uint16_t major=hb_explicit_scale_mask(source_root,1);
    uint16_t minor=hb_explicit_scale_mask(source_root,2);
    int major_score=hb_popcount12((uint16_t)(chord&major));
    int minor_score=hb_popcount12((uint16_t)(chord&minor));
    return minor_score>major_score?2:1;
}
...
static int hb_nth_parent_scale_interval_from_root(uint16_t parent_mask,int target_root,int degree){
    /* Return the semitone interval from target_root to the requested ordinal
       degree of the parent scale when that scale is viewed from target_root.
       Example F major viewed from C: C D E F G A Bb -> Mixolydian. */
    if(degree<=0)return 0;
    int seen=0;
    for(int semitones=1;semitones<=24;semitones++){
        int pc=mod12(target_root+semitones);
        if(parent_mask&(1u<<pc)){
            seen++;
            if(seen==degree)return semitones;
        }
    }
    return degree*2; /* defensive fallback */
}
static int hb_map_scale_degree_relative(Inst *instance,int source_note,hb_harmony_t harmony){
    ...
    int degree=hb_source_interval_to_diatonic_degree(mod12(source_note-source_root));
    int parent_scale_index=hb_parent_scale_index(instance,harmony);
    uint16_t parent_mask=hb_explicit_scale_mask(source_root,parent_scale_index);

    /* If the detected chord root is outside the parent scale, preserve the
       diatonic-role idea using a scale inferred from that chord quality rather
       than silently reverting to chromatic Free behavior. */
    uint16_t target_scale_mask=parent_mask;
    if(!(parent_mask&(1u<<mod12(harmony.root_pc)))){
        hb_harmony_t fallback=harmony;
        fallback.pitch_mask=hb_scale_mask(harmony);
        target_scale_mask=fallback.pitch_mask;
    }

    int target_interval=hb_nth_parent_scale_interval_from_root(target_scale_mask,harmony.root_pc,degree);
...
static int hb_map_follower_note_now(Inst *instance,int source_note){
    ...
    if(!harmony.valid||hb_global_content_map()==2)return source_note; /* Free */

    /* In Scale + Relative transports the follower's degree from its reference
       root to the DETECTED harmony root. Do this before constructing a target
       scale mask: an explicit follower scale describes source-degree semantics,
       not a replacement root for the conductor harmony. */
    if(hb_global_content_map()==1&&hb_global_travel_map()==0)
        return hb_map_scale_degree_relative(instance,source_note,harmony);

    if(hb_global_content_map()==1)harmony=hb_follower_scale_target(instance,harmony);
    else harmony=hb_mapping_target(harmony,0);
    if(hb_global_travel_map()==1)
        return hb_map_note(source_note,reference_root(instance),harmony,HB_MAP_NEAREST);
    if(hb_global_travel_map()==2)
        return hb_map_note_upward(source_note,harmony);
    int used_root=0;
    if(!hb_resolve_follower_reference_root(instance,&used_root))return source_note;
    return hb_map_note(source_note,used_root,harmony,HB_MAP_CHORD);
}
...
static hb_harmony_t hb_follower_scale_target(Inst *instance,hb_harmony_t harmony){
    ...
    if(hb_global_scale()==0){
        harmony.pitch_mask=hb_scale_mask(harmony);
        return harmony;
    }
    int root=0;
    if(!hb_resolve_follower_reference_root(instance,&root))root=harmony.root_pc;
    harmony.root_pc=mod12(root);
    harmony.bass_pc=harmony.root_pc;
    harmony.pitch_mask=hb_explicit_scale_mask(harmony.root_pc,hb_global_scale());
    return harmony;
}
static hb_harmony_t hb_mapping_target(hb_harmony_t harmony,int map_target){
    if(!harmony.valid)return harmony;
...
"]";
static void set_param(void *value,const char *key,const char *parameter){Inst *instance=(Inst*)value;if(!instance||!key||!parameter)return;if(!strcmp(key,"live_press")){if(parameter[0]=='1')hb_receive_live_vouch(instance);return;}if(!strcmp(key,"track_role")||!strcmp(key,"role")){
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    int is_hb14=(parsed==24);
    if(!is_hb14)parsed=sscanf(state,"hb13,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
        &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
        &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
        &values[12],&values[13],&values[14],&values[15],&values[16],&values[17],
        &values[18],&values[19],&values[20],&values[21],&values[22],&values[23]);
    int is_hb13=(!is_hb14&&parsed==24);
    if(!is_hb14&&!is_hb13)parsed=sscanf(state,"hb12,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
        &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
        &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
        &values[12],&values[13],&values[14],&values[15],&values[16],&values[17],
        &values[18],&values[19],&values[20],&values[21],&values[22]);
    int is_hb12=(!is_hb13&&parsed==23);
    if(!is_hb14&&!is_hb13&&!is_hb12)parsed=sscanf(state,"hb11,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
        &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
        &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
        &values[12],&values[13],&values[14],&values[15],&values[16],&values[17],
        &values[18],&values[19],&values[20]);
    int is_hb11=(!is_hb13&&!is_hb12&&parsed==21);
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11)parsed=sscanf(state,"hb10,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
        &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
        &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
    ...
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10)parsed=sscanf(state,"hb9,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
        &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
        &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
    ...
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9)parsed=sscanf(state,"hb8,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
        &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
        &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
        &values[12],&values[13],&values[14],&values[15],&values[16],&values[17]);
    if(parsed==18){
    ...
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9&&parsed!=18){
        parsed=sscanf(state,"hb7,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
            &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
            &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
            &values[12],&values[13],&values[14],&values[15]);
    }
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9&&parsed!=16&&parsed!=18){
        parsed=sscanf(state,"hb6,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
            &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
            &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
            &values[12],&values[13],&values[14]);
    }
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9&&parsed!=15&&parsed!=16&&parsed!=18){
        parsed=sscanf(state,"hb5,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
            &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
            &values[6],&values[7],&values[8],&values[9],&values[10],&values[11],
            &values[12],&values[13],&values[14],&values[15]);
    }
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9&&parsed!=16&&parsed!=15&&parsed!=18){
    ...
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9&&parsed!=13&&parsed!=15&&parsed!=16&&parsed!=18){
        parsed=sscanf(state,"hb2,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
            &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
            &values[6],&values[7],&values[8],&values[9],&values[10],&values[11]);
    }
    if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9&&parsed!=12&&parsed!=13&&parsed!=15&&parsed!=16&&parsed!=18){
    ...
    if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9||parsed>=12)&&values[11]>=-1&&values[11]<16)instance->render_channel=values[11];
    if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9||parsed==13||parsed>=15)&&values[12]>=-1&&values[12]<16)instance->source_channel=values[12];
    if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9||parsed>=18)&&values[16]>=0&&values[16]<=100)instance->follow_lookahead_ms=values[16];
    if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9||parsed>=18)&&values[17]>=0&&values[17]<=1)instance->retrigger_held=values[17];
    ...
    if(is_hb14&&values[0]==0&&!g_follower_globals_restored){
        /* Seed global follower settings once from the saved conductor snapshot.
           After that, only explicit user changes may alter them. This prevents
           UI instance recreation / stale per-track state from snapping them
           back or leaving the enum with no matching value. */
        if(values[21]>=0&&values[21]<3)hb_set_global_content_map(values[21]);
        if(values[22]>=0&&values[22]<3)hb_set_global_travel_map(values[22]);
        if(values[23]>=0&&values[23]<10)hb_set_global_scale(values[23]);
        g_follower_globals_restored=1;
    }
    if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10||is_hb9)&&values[0]==0&&!instance->global_timing_restored){
        ...
        if((is_hb14||is_hb13||is_hb12||is_hb11||is_hb10)&&values[19]>=0&&values[19]<=500)g_bus.boundary_buffer_ms=values[19];
        if((is_hb14||is_hb13||is_hb12||is_hb11)&&values[20]>=0&&values[20]<=500)g_bus.analysis_release_ms=values[20];
        instance->global_timing_restored=1;
    }else if(!is_hb14&&!is_hb13&&!is_hb12&&!is_hb11&&!is_hb10&&!is_hb9&&parsed>=15&&values[0]==0&&!instance->global_timing_restored){
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB235");
