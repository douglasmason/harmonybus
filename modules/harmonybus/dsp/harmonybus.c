...
static double hb_next_shift_beats(void){
...
static int hb_next_allows_precapture(void){
    return hb_next_lookahead_beats()<=0.0||g_bus.next_anti_buffer_ms==0;
}
...
static int hb_next_model_event_for_phase(double phase,int shifted){
    ...
    double length=hb_next_loop_length();
    if(length<=0.0)return -1;
    double lookahead=shifted?hb_next_shift_beats():0.0;
...
static double hb_next_effective_boundary(double absolute_beat){
    ...
    double phase=hb_next_phase(hb_clip_playhead());
    double lookahead=hb_next_shift_beats();
    double best=1e99;
    for(int index=0;index<g_bus.next_model_count;index++){
...
static int hb_queue_follower_event(Inst *instance,int note,int velocity,int is_on,int channel){
    ...
    memcpy(instance->motion_follower_events[slot],instance->motion.events,sizeof(instance->motion.events));
    ...
    if(is_on){
        ...
        double target;
        /* Musical labels describe nominal divisions. Leave a 1 ms margin at
           the leading edge so aligned notes cannot enter the next window. */
        if(g_bus.boundary_buffer_ms<0){
            capture-=hb_ms_to_beats(1);
        ...
        if(learned_boundary>=0.0){
            ...
            if(distance>=-1e-6&&distance<=capture+1e-6&&
               (hb_next_allows_precapture()||distance<=1e-6)){
...
static int hb_parent_scale_index(Inst *instance,hb_harmony_t harmony){
    ...
    return hb_inferred_parent_scale_index(source_root,hb_transpose_harmony(harmony,-g_bus.global_transpose));
}
...
static uint16_t hb_output_chord_scale(Inst *instance,hb_harmony_t harmony,int source_root,uint16_t parent){
    ...
    parent=hb_transpose_mask(parent,g_bus.global_transpose);
...
static int hb_map_follower_note_relative(Inst *instance,int source_note,hb_harmony_t detected,
    ...
    int nominal=source_note+g_bus.global_transpose+hb_signed_root_delta(source_root,mod12(detected.root_pc-g_bus.global_transpose));
...
static int hb_map_follower_note_closest_split(Inst *instance,int source_note,hb_harmony_t detected,
    ...
    if(instance->follower_split_map==3){
        ...
        for(int index=0;index<active_count;index++)
            active_mask|=(uint16_t)(1u<<mod12(active_notes[index]+g_bus.global_transpose));
    }
    ...
    for(int degree=0;degree<HB_CLOSEST_SPLIT_DEGREES;degree++){
        ...
        nominal_by_degree[degree]=source_root_note+source_interval+g_bus.global_transpose;
...
static int hb_map_follower_note_now(Inst *instance,int source_note){
    ...
    if(!detected.valid)return hb_cp_clamp(source_note+g_bus.global_transpose,0,127);
    ...
    if(travel==1)
        return hb_play_note(instance,hb_map_note(hb_cp_clamp(source_note+g_bus.global_transpose,0,127),reference_root(instance),target,HB_MAP_NEAREST),detected,target.pitch_mask);
    if(travel==2)
        return hb_play_note(instance,hb_map_note_upward(hb_cp_clamp(source_note+g_bus.global_transpose,0,127),target),detected,target.pitch_mask);
    if(travel==4)
        return hb_play_note(instance,hb_map_note_downward(hb_cp_clamp(source_note+g_bus.global_transpose,0,127),target),detected,target.pitch_mask);
    return source_note;
}
...
static void hb_motion_harmony_refresh(Inst *instance){
    ...
    if((active||instance->motion_harmony_signature)&&signature!=instance->motion_harmony_signature)instance->play_revision++;
    instance->motion_harmony_signature=active?signature:0;
}
...
if(instance){
    ...
    instance->used=0;
}for(int index=0;index<HB_MAX_INSTANCES;index++)if(g_pool[index].used)return;g_buffer_restored=0;g_bus.boundary_buffer_ms=-3;g_lookahead_restored=0;g_bus.next_lookahead=0;g_bus.next_anti_buffer_ms=25;hb_pad_defaults();}
...
static void hb_clear_instance_note_state(Inst *instance){
    if(!instance)return;
    instance->motion.held=0;instance->motion.pitch_held=0;instance->motion.enclosure=0;
...
if(!strcmp(key,"next_lookahead")){g_lookahead_restored=1;g_bus.next_lookahead=enum_index(parameter,NEXT_LOOKAHEAD_OPTS,25,g_bus.next_lookahead);if(g_bus.next_predict&&g_bus.next_model_locked)hb_next_apply_effective(hb_clip_playhead());return;}
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if(!g_lookahead_restored&&lookahead_suffix&&sscanf(lookahead_suffix,";la1,%d,%d",&lookahead,&anti_buffer)==2&&
    ...
    if(ready){
        ...
        if(instance->player.config.playback!=1){
            ...
            if(boundary>=now-1e-6&&boundary-now<=capture+1e-6&&
               (hb_next_allows_precapture()||boundary-now<=1e-6))
