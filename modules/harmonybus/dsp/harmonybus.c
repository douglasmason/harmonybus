/* Harmony Bus v0.2.52 — Schwung MIDI FX. */
#define HB_VERSION "0.2.52"
    ...
    if(is_on){
        ...
        for(int index=instance->follower_queue_count-2;index>=0;index--){
            ...
            if(instance->follower_queue_target_beat[index]>=0.0){
                queued_on_target=instance->follower_queue_target_beat[index];
                double queued_delay=queued_on_target-instance->follower_queue_arrival_beat[index];
                if(queued_delay>delay)delay=queued_delay;
            }
            break;
        }
        if(queued_on_target>=0.0&&beat<queued_on_target){
            /* A short note can be released physically before its delayed ON.
               Do NOT delay the OFF again from the OFF arrival time: that can
               make large buffers behave as if the hit vanished or leave a
               badly stretched note. Serialize ON then OFF at the same target;
               queue order guarantees ON is emitted first. */
            instance->follower_queue_target_beat[slot]=queued_on_target;
        }else if(delay>0.0){
            instance->follower_queue_target_beat[slot]=beat+delay;
        }
    }
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB252");
