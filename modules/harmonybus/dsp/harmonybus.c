...
static int hb_sync_from_monitor(Inst *instance){
    ...
    if(instance->role==0){
        ...
        if(changed){
            instance->follower_source_harmony=hb_resolve_follower_source_harmony(instance);
            for(int note=0;note<128;note++){
                instance->follower_role_interval[note]=instance->follower_held[note]&&instance->follower_source_harmony.valid
                    ?(uint8_t)mod12(note-instance->follower_source_harmony.root_pc):255;
            }
