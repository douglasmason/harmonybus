...
static int hb_observed_notes(const Inst *instance,uint8_t *output,int max_notes){
    ...
    int allow_realtime=(g_bus.sensor_sources==0||g_bus.sensor_sources==1);
    int allow_clip=(g_bus.sensor_sources==1||g_bus.sensor_sources==2);
    if(allow_realtime){
        for(int note=0;note<128&&count<max_notes;note++){
            if(instance->held_now[note]&&!seen[note]){
                seen[note]=1;output[count++]=(uint8_t)note;
            }
        }
    }
    if(allow_clip&&g_bus.clip_valid&&g_bus.clip_note_count>0){
...
static int hb_reharmonize_held_follower(Inst *instance,uint8_t output[][3],int lengths[],int max_output){
    ...
    if(instance->mode==HB_MAP_CHORD&&instance->follower_source_harmony.valid)
        hb_map_held_voices_from_harmony(source_notes,voice_count,
                                        instance->follower_source_harmony,bus_read(),new_outputs);
    else
