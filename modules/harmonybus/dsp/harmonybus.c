/* Harmony Bus v0.2.04 — Schwung MIDI FX. */
#define HB_VERSION "0.2.04"
...
static int hb_source_channel_matches(Inst *instance,int midi_channel){
    ...
    /* Auto is keyed by the parent Chain recv channel supplied by the
       source-aware Chain bridge. This is the stable per-track identity.
       Older bridges do not tag it, so retain the legacy accept-all fallback
       rather than binding permanently to whichever broadcast event arrives first. */
    if(instance->resolved_source_channel>=0&&instance->resolved_source_channel<16)
        return midi_channel==instance->resolved_source_channel;
    return 1;
}
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB204");
