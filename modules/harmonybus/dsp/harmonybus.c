/* Harmony Bus v0.2.13 — Schwung MIDI FX. */
#define HB_VERSION "0.2.13"
...
static int hb_find_conductor_track(const char *uuid){
    ...
    for(int track=0;track<4;track++){
        ...
        int is_conductor=(strstr(json,"hb10,0,")||strstr(json,"hb9,0,")||strstr(json,"hb8,0,")||strstr(json,"hb7,0,")||strstr(json,"hb6,0,")||strstr(json,"hb5,0,")||strstr(json,"hb4,0,")||strstr(json,"hb3,0,")||strstr(json,"hb2,0,")||strstr(json,"hb1,0,"));
        free(json);
...
static void hb_restore_state(Inst *instance,const char *state){
    ...
    if(!is_hb10&&!is_hb9&&parsed!=13&&parsed!=15&&parsed!=16&&parsed!=18){
        parsed=sscanf(state,"hb2,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
            &values[0],&values[1],&values[2],&values[3],&values[4],&values[5],
            &values[6],&values[7],&values[8],&values[9],&values[10],&values[11]);
    }
    if(!is_hb10&&!is_hb9&&parsed!=12&&parsed!=13&&parsed!=15&&parsed!=16&&parsed!=18){
    ...
    if((is_hb10||is_hb9||parsed>=12)&&values[11]>=-1&&values[11]<16)instance->render_channel=values[11];
    if((is_hb10||is_hb9||parsed==13||parsed>=15)&&values[12]>=-1&&values[12]<16)instance->source_channel=values[12];
    if((is_hb10||is_hb9||parsed>=18)&&values[16]>=0&&values[16]<=100)instance->follow_lookahead_ms=values[16];
    if((is_hb10||is_hb9||parsed>=18)&&values[17]>=0&&values[17]<=1)instance->retrigger_held=values[17];
    if((is_hb10||is_hb9)&&values[0]==0){
        /* Global harmony timing belongs to the shared bus, not to each track.
           Restore it only from the Conductor instance. Otherwise merely
           visiting/restoring a Follower track can overwrite the live global
           value with that track's older serialized copy. */
        ...
        if(is_hb10&&values[19]>=0&&values[19]<=100)g_bus.boundary_buffer_ms=values[19];
    }else if(!is_hb10&&!is_hb9&&parsed>=15){
...
if(!strcmp(key,"v87_marker"))return snprintf(buffer,(size_t)length,"%s","HB213");
