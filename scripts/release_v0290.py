...
# Prediction is always learning/available. Lookahead=Off is the bypass, so the
# redundant Predict switch disappears from the panel and chain params.
...
next_panel["params"] = [
    param for param in next_panel["params"] if param.get("key") != "next_predict"
]
next_panel["knobs"] = [
...
# v0.2.88/v0.2.89 normalizers could append the same Next Harm get_param block
# on repeated runs. Collapse all repeated copies to one clean block.
...
source = source.replace(get_anchor, get_anchor + get_block, 1)

# Retain next_predict only as a compatibility field for old state/UI callers,
# but force it on. Lookahead=Off is now the only bypass.
...
# Add a beat-duration accumulator. This is intentionally based on tick frames
# and BPM, not transport/playhead wrap detection. A contiguous loop-length of
# playback visits every circular clip phase exactly once regardless of where
# learning starts.
...
# Replace wrap-gated promotion with promotion of any full-duration observation
# window. Event phases are clip-local, so chronological starting phase does not
# matter and the event list need not begin at loop phase zero.
...
promote_new = r'''static void hb_next_promote_learning(void){
    ...
    if(g_bus.next_learning_count<=0&&g_bus.observed_harmony.valid){
        /* Constant-harmony loop: there was no transition to record. */
...
# Repeated normalizers also left duplicate update calls. Keep exactly one call
# and pass tick timing so learning progress is independent of wrap detection.
...
# Preserve the last valid playhead on stop so Position shows where playback
# actually stopped. Only invalidate the predictor's continuity sample.
...
# Make old/hidden Predict writes harmless and permanently On.
source = re.sub(
