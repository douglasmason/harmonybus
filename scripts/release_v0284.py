...
module["version"] = "0.2.84"
module["name"] = "Harmony Bus 0.2.84"
module["abbrev"] = "HB284"
module["description"] = "Harmony Bus v0.2.84 — scale ornaments independent of Content; seven-degree monotonic Closest Split"
module["capabilities"]["ui_hierarchy"]["levels"]["root"]["name"] = "Harmony Bus 0.2.84"
MODULE.write_text(json.dumps(module, indent=2) + "\n")

source = DSP.read_text()
source = re.sub(r'/\* Harmony Bus v0\.2\.\d+ — Schwung MIDI FX\. \*/',
                '/* Harmony Bus v0.2.84 — Schwung MIDI FX. */', source, count=1)
source = re.sub(r'#define HB_VERSION "0\.2\.\d+"', '#define HB_VERSION "0.2.84"', source, count=1)
