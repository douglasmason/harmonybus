...
rm -rf "$OUT" "$ROOT/dist/harmonybus" "$ROOT/dist/harmonybus-v0.1.86-module.tar.gz"
mkdir -p "$OUT" "$DIST"
...
chmod +x "$DIST/dsp.so"
(cd "$ROOT/dist" && tar -czvf harmonybus-v0.1.86-module.tar.gz harmonybus/)
echo "$ROOT/dist/harmonybus-v0.1.86-module.tar.gz"
