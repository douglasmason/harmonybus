...
rm -rf "$OUT" "$DIST_HB" "$DIST_MON" \
  "$ROOT/dist/harmonybus-v0.2.60-module.tar.gz" \
  "$ROOT/dist/harmonybus-monitor-v0.2.60-tool.tar.gz"
...
(cd "$ROOT/dist" && tar -czvf harmonybus-v0.2.60-module.tar.gz harmonybus/)
...
tar -C "$DIST_MON" -czvf "$ROOT/dist/harmonybus-monitor-v0.2.60-tool.tar.gz" module.json ui.js dsp.so

echo "$ROOT/dist/harmonybus-v0.2.60-module.tar.gz"
echo "$ROOT/dist/harmonybus-monitor-v0.2.60-tool.tar.gz"
